...
void main() {
  group('Input Data', () {
    ...
    test("password without uppercases", () {
      expect(
          validatePassword(generatePassword(false, true, true, true)), false);
    });
    test("password without lowercases", () {
      expect(
          validatePassword(generatePassword(true, false, true, true)), false);
    });
    test("password without digits", () {
      expect(
          validatePassword(generatePassword(true, true, false, true)), false);
    });
    test("password without underscore", () {
      expect(
          validatePassword(generatePassword(true, true, true, false)), false);
    });
    test("password without upper and lower cases", () {
      expect(
          validatePassword(generatePassword(false, false, true, true)), false);
    });
    test("password without upper cases and digits", () {
      expect(
          validatePassword(generatePassword(false, true, false, true)), false);
    });
    test("password without upper and underscore", () {
      expect(
          validatePassword(generatePassword(false, true, true, false)), false);
    });
    test("password without lower cases and digits", () {
      expect(
          validatePassword(generatePassword(true, false, false, true)), false);
    });
    test("password without lower cases and underscore", () {
      expect(
          validatePassword(generatePassword(true, false, true, false)), false);
    });
    test("password without digits and underscore", () {
      expect(
          validatePassword(generatePassword(true, true, false, false)), false);
    });
    test("password without upper, lower cases and underscore", () {
      expect(
          validatePassword(generatePassword(false, false, true, false)), false);
    });
    test("password with underscore only", () {
      expect(
          validatePassword(generatePassword(false, false, false, true)), false);
    });
    ...
    test("password with lowercases only", () {
      expect(
          validatePassword(generatePassword(false, true, false, false)), false);
    });
    test("password with uppercase only", () {
      expect(
          validatePassword(generatePassword(true, false, false, false)), false);
    });
    ...
    test("valid national id for Egypt", () {
      ...
      expect(parser.validateNationalID(generateNationalID(true, true, true)),
          true);
    });
    ...
    test("invalid national id in year for Egypt", () {
      NationalIDParser parser = EgyptNationalIDParser();
      expect(parser.validateNationalID(generateNationalID(true, false, true)),
          false);
    });
    test("invalid national id in days/month for Egypt", () {
      NationalIDParser parser = EgyptNationalIDParser();
      expect(parser.validateNationalID(generateNationalID(true, true, false)),
          false);
    });
    test("invalid national id in days/month and years for Egypt", () {
      NationalIDParser parser = EgyptNationalIDParser();
      expect(parser.validateNationalID(generateNationalID(true, false, false)),
          false);
    });
