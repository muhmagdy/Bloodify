...
String generateInvalidName() {
  ...
  int length = random.nextInt(30) + 20,
      definiteInvalidIndex = random.nextInt(length),
      charCode = 0;
  for (int i = 0; i < length; i++) {
  ...
  result = result.substring(0, definiteInvalidIndex) +
      String.fromCharCode(charCode) +
      result.substring(definiteInvalidIndex, length);
  return result;
}
...
String generatePassword(
    bool upperReq, bool lowerReq, bool digitReq, bool _req) {
  String result = "";
  Random random = Random();
  ...
  for (int i = 0; i < length; i++) {
    ...
    if ((digitReq && charCode >= 48 && charCode <= 57) ||
        (upperReq && charCode >= 65 && charCode <= 90) ||
        (lowerReq && charCode >= 97 && charCode <= 122) ||
        (_req && charCode == 95)) {
      result += String.fromCharCode(charCode);
    }
  }
...
generateInvalidPassword() {
  ...
  String password = generatePassword(random.nextBool(), random.nextBool(),
      random.nextBool(), random.nextBool());
  int ind = random.nextInt(password.length + 1);
  password =
      password.substring(ind) + invalidSubString + password.substring(0, ind);
...
int generateNationalID(bool validCentury, bool validYear, bool validDay) {
  ...
  int year = generateYear(validYear, centuryNumber),
