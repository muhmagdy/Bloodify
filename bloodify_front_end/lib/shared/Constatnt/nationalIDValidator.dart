...
class EgyptNationalIDParser extends NationalIDParser {
  ...
  bool validateNationalID(int number) {
    ...
    if (centuryNumber < minCenturyNum || centuryNumber > maxCenturyNum)
      return false;
    centuryNumber -= 2;
    ...
    if (days == 0 || !(thirty_Days_Month || thirty_one_Days_Month || february))
      return false;
  ...
  bool Thirty_Days_Month(int months, int days) {
    return (months == 9 || months == 4 || months == 6 || months == 11) &&
        days <= 30;
  }
  ...
  bool February(int years, int months, int days) {
    if (months == 2 &&
        ((isLeapYear(years) && days <= 29) ||
            (!isLeapYear(years) && days <= 28))) return true;
    return false;
  }
